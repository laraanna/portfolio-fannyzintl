  ...
  return (
    <main className="mx-auto w-[95%] 2xl:w-[70%] px-9 pt-[120px] pb-6 space-y-12">
      ...
      <section id="section-services" className="pb-32 xl:pb-70 md:max-w-[70%] xl:max-w-[50%]">
       ...
       <div className="flex gap-[25px] 2xl:gap-[50px]">
        <div className="flex-1">
          {services.slice(0, 6).map((item, index) => (
            <div key={item} className={`py-3 text-[0.68rem] md:text-sm xl:text-xl 2xl:text-2xl border-b border-neutral-300 lg:max-w-[200px] xl:max-w-[300px] 2xl:max-w-[400px] ${index === 0 ? 'border-t' : ''}`}>
              {item}
            </div>
          ))}
        </div>
        <div className="flex-1">
          {services.slice(6, 12).map((item, index) => (
            <div key={item} className={`py-3 text-[0.68rem] md:text-sm xl:text-xl 2xl:text-2xl border-b border-neutral-300 lg:max-w-[220px] xl:max-w-[300px] 2xl:max-w-[400px] ${index === 0 ? 'border-t' : ''}`}>
              {item}
            </div>
          ))}
        </div>
      ...
      <section id="section-contact" className="md:max-w-[70%] lg:max-w-[40%] md:ml-[15%] lg:ml-[30%]">
        ...
        <p className="text-sm/4.5 xl:text-lg 2xl:text-1_5xl mb-4">I work with brands and teams on projects of all sizes. If you’re looking for a strategic partner or are building something, send me a message. </p>
        <p className="text-sm/4.5 xl:text-lg 2xl:text-1_5xl">info.fannyzintl@gmail.com / <a href="https://www.linkedin.com/in/fanny-zintl-ab183ba7/" target="_blank" className="border-b border-black" rel="noopener noreferrer">LinkedIn</a></p>
      </section>
    </main>
  );
}
